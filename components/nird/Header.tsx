...
export default function Header() {
  ...
  return (
    ...
    >
        ...
        <div className="flex items-center justify-between h-16">
          ...
          <div className="hidden lg:flex items-center gap-3">
            <a
              ...
              className={`px-4 py-2 text-sm font-medium transition-colors flex items-center gap-2 ${isScrolled ? "text-gray-600 hover:text-green-600" : "text-white/80 hover:text-white"
                }`}
            >
              Village Résistant
            </a>
            <button
              ...
              className={`px-4 py-2 text-sm font-medium transition-colors ${isScrolled ? "text-gray-600 hover:text-purple-600" : "text-white/80 hover:text-white"
                }`}
            >
              Contact
            </button>
          ...
          <button
            ...
            className={`lg:hidden w-10 h-10 flex items-center justify-center rounded-lg transition-colors ${isScrolled ? "hover:bg-gray-100" : "hover:bg-white/10"
              }`}
            aria-label="Menu"
          >
      ...
      {/* Mobile Menu */}
      <div
        className={`lg:hidden bg-white border-t border-gray-100 transition-all duration-300 overflow-hidden ${isMobileMenuOpen ? "max-h-96 opacity-100" : "max-h-0 opacity-0"
          }`}
      >
            ...
            {navItems.map((item) => {
              ...
              return (
                <button
                  key={item.label}
                  onClick={() => handleNavClick(item.href)}
                  className={`px-4 py-3 rounded-lg text-left font-medium transition-colors ${isActive
